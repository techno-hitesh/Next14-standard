import { deleteadminsubCategoryApi} from '@/app/services/apis/admin/products'
import { useRouter } from 'next/navigation'
import React, { useState } from 'react'
import { toast } from 'react-toastify'

const SubCategoryDelete = ({ id }: { id: { id: any | string } }) => {
       const [showModal,setShowModal]=useState(false)
       const router=useRouter()
       const handleDelete=async()=>{
             setShowModal(false)
             const response=await deleteadminsubCategoryApi(id)
...
{/* <!-- Main modal --> */}
{showModal ? 
<div id="deleteModal"   aria-hidden="true" className="  top-[50%]   overflow-y-auto overflow-x-hidden fixed bg-gray-200 bg-opacity-30 right-0 left-0  z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
    <div className="relative left-[30%]   top-[30%] p-4 w-full max-w-md h-full md:h-auto">
        {/* <!-- Modal content --> */}
